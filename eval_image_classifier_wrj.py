...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 23:23:20 2017
...
def main():
  ...
  with tf.Graph().as_default():
    ...
    image1 = image_preprocessing_fn(image, eval_image_size, eval_image_size)
...
#        eval_op=list(names_to_updates.values())
        for i in range(60):
            logits_ , soft_result_, labels_, predictions_= sess.run([logits, soft_result, labels, predictions])
            for m in range(FLAGS.batch_size):
                for n in range(30):
                    tag.append([labels_[m],str(n+1),str('%.8f'%(soft_result_[m,n]))])
